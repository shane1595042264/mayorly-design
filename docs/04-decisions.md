...
### The ledger shelf is 32x64 and carries bays the game fills
The founder rejected the bookshelf wireframe as bare and asked how much detail he may add without breaking the code that will place ledgers on it. The ledger spines he has already drawn are 6x13; three per shelf need a bay of 22x14, and three such bays plus a Stardew-style crown, planks and plinth do not fit in 48 rows. **Decided:** a new `prop2x4` class (32x64, one class, one slot), and slots may declare `bays`, rectangles the game draws into. Inside a bay the art must be a flat back panel; outside, everything is the artist's. Both rules are validator-enforced (`bays.fit`, `bays.flat`) and the rects ship in the manifest so the game reads them rather than hardcoding. The Mayor's Hall walls are therefore at least four tiles tall. **Rejected:** shrinking the ledger icons (finished art, and the 13px spine is the click target), and painting decorative filler books into the bays (muddles the nine-ledger cap). **Reverses if:** the four-tall shelf dwarfs the room once it is on screen; the fallback is a 3x3-tile shelf with two bays of five and four.
